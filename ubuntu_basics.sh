...
function updateAlias { # tested on lubuntu 15.04
	sudo echo "alias update='sudo apt-get update && if [ -n \"\$(which npm)\" ]; then sudo npm install -g npm; fi && sudo apt-get upgrade -y && sudo apt-get dist-upgrade -y && sudo apt-get autoremove -y && do-release-upgrade'" >> ~/.bashrc
}
